...
--EXEC INSERT_ITEM @strAccountID, 2, 800060000, 1

----------------------------------
-- EXEC GIVE_PREMIUM 'CharacterName', '', Days
----------------------------------
	...
	BEGIN 
		...
		ELSE IF @strUserID IS NOT NULL AND @strUserID <> ''
			BEGIN
				...
				SET @strAccountID = (SELECT strAccountID FROM ACCOUNT_CHAR WHERE strCharID1 = @strUserID or strCharID2 = @strUserID or strCharID3 = @strUserID)
				SELECT @lastDate = PremiumExpire FROM TB_USER WHERE strAccountID = @strAccountID
				SET @dateDiff = DateDiff(dd, getDate(), @lastDate)
				IF @lastDate IS NULL or @dateDiff <= 0
					BEGIN
